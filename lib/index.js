	...
	ctx.sessionProjections.register({
		...
		apply(state, event) {
			if (event.type === "user/message") {
				...
				if (msg && msg.source && msg.source.kind === "plugin" && msg.source.plugin === "dsh-model-router") return {
					...state,
			...
			try {
				...
				if (result.stopReason === "completed" && output.length > 0) {
					...
					payload.agent.inject({
						...
						content: [{
							type: "text",
							text: "A quick-answer subagent already answered the question above in an isolated conversation. Reply to the user with that answer exactly as written — do not re-answer, add analysis, or call tools."
						}, ...output],
						source: {
							kind: "plugin",
