...
function apply(ctx) {
	...
	*/
	async function flashJudge(provider, model, question, signal) {
		...
		return blocks.join(" ").trim().toUpperCase().startsWith("SIMPLE") ? "cheap" : "default";
	}
	...
	ctx.on("agent/pre-step", async (payload, next) => {
		...
		if (classify(payload.messages) === "strong") {
		...
		try {
			...
			if (await flashJudge(provider, catalog.cheap, question, payload.signal) !== "cheap") return next();
