...
`;
		...
		function apply(ctx) {
			...
			function RouterDock(props) {
				...
				const setMode = (next) => {
					setBusy(true);
					(ctx.remote && ctx.remote.commands ? ctx.remote.commands.execute(sessionId, "/router " + next) : Promise.reject(/* @__PURE__ */ new Error("commands remote unavailable"))).catch((error) => console.error("dsh-model-router: set-mode failed: " + String(error))).finally(() => setBusy(false));
				};
