...
`;
		...
		function apply(ctx) {
			...
			function RouterDock(props) {
				...
				const cacheHitRate = totals && totals.inTokens > 0 ? Math.round(totals.cacheRead / totals.inTokens * 100) : 0;
				const setMode = (tier) => {
					setBusy(true);
				...
				return (0, react.createElement)("div", { className: "mrtr" }, (0, react.createElement)("div", { className: "mrtr-bar" }, (0, react.createElement)("span", { className: "mrtr-label" }, "⚡Router"), modeBtn("auto", "Auto", "heuristic routing: trivial steps run on the cheap model"), modeBtn("cheap", "省", "force the cheapest catalog model"), modeBtn("strong", "强", "force the strongest catalog model"), current ? (0, react.createElement)("span", {
					...
					className: "mrtr-meta",
					title: "estimated cost; prices are configurable estimates"
				}, "in " + fmt(totals.inTokens) + " · out " + fmt(totals.outTokens) + " · cache " + cacheHitRate + "% · ≈$" + fmtCost(totals.cost)) : null), Object.keys(byModel || {}).length > 0 ? (0, react.createElement)("details", { className: "mrtr-details" }, (0, react.createElement)("summary", null, "usage (" + (totals ? totals.calls : 0) + " calls)"), modelRows) : null);
			}
